...
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            #
            #     # ==========================================================
            #     # 修改点：将双向复制粘贴替换为伪标签背景涂黑
            #     # ==========================================================
            #     # 1. 提取无标签的前景掩码 (>0 的区域)
            #     fg_mask_a = (plab_a > 0).float().unsqueeze(1)
            #
            #     # 2. 伪造原版函数的 mask 格式 (全1)，无缝兼容后面坚决不改的 mix_loss
            ...
            #
            #     unl_label = plab_a  # 保留用于后续 Tensorboard 可视化
            ...
            # # 修改点：改变送入网络的图像
            # # ==========================================================
            # net_input_unl = uimg_a * fg_mask_a  # 无标签图：绝对涂黑背景
            # net_input_l = img_b  # 有标签图：保持原样提供干净监督
            # # ==========================================================

                # ==========================================================
                # 修改点：将双向复制粘贴/背景涂黑 替换为【全图随机盲切 Patch】
                # ==========================================================
                B, C, H, W = uimg_a.shape
                patch_size = 128  # 孤岛的大小
                num_patches = 1  # 随机保留的孤岛数量 (可自由修改)
