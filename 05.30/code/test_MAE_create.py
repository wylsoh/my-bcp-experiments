...
parser.add_argument('--exp', type=str, default='BCP_MAE_create', help='experiment_name')
...
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    # 两阶段微调控制参数初始化
    # =========================================================================
    stable_iter = 10000  # 设定模型输出稳定断点步数
    is_frozen_stage = False
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # =========================================================================
            # 【一针见血控制一】：动态触发网络梯度解耦与冻结切换
            # =========================================================================
            if iter_num >= stable_iter and not is_frozen_stage:
                logging.info(f"============= ITERATION {iter_num} REACHED STABLE BREAKPOINT =============")
                logging.info("--> Action: Freezing front layers, optimizing last 2 layers.")

                for name, param in model.named_parameters():
                    if "encoder" in name or "Swin" in name or "backbone" in name:  # 适配主流骨干网络名称
                        param.requires_grad = False
                    else:
                        param.requires_grad = True  # 让整个解码器和最后两层形成完整的可导通路

                # 重新构建只包含最后两层参数的微调优化器
                optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=base_lr, momentum=0.9,
                                      weight_decay=0.0001)
                is_frozen_stage = True

            with torch.no_grad():
                # =========================================================================
                # 【一针见血控制二】：用 MAE 盲切残缺视野在前向反馈时生成互换伪标签
                # =========================================================================
                if is_frozen_stage:
                    # 第二阶段：模型已稳定，启动多尺度方格物理盲切机制
                    ...
                    # 使用经过多尺度随机方格裁剪后的【残缺盲切图像输入】迫使教师前向生成 MAE 伪标签
                    uimg_a_mae_input = uimg_a * random_patch_mask
                    pre_a = ema_model(uimg_a_mae_input)
                    ...
                    # 构建默认完全放行的平面占位掩码
                    loss_mask = torch.ones(uimg_a.shape[0], uimg_a.shape[2], uimg_a.shape[3]).cuda().long()
                    random_patch_mask = torch.ones_like(uimg_a)
                    plab_a_masked = plab_a
                    scale_multiplier = 1.0

                probs_a = F.softmax(pre_a, dim=1)
                teacher_certainty = calculate_entropy_certainty(probs_a)
                dynamic_u_weight = args.u_weight * scale_multiplier * teacher_certainty.item()
            ...
            if iter_num % 20 == 0:
                ...
                labs = plab_a_masked[1, ...].unsqueeze(0) * 50  # 修正：变量对齐解决 NameError 隐患
