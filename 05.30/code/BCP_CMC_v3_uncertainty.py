"""
BCP + CMC v3：互补视图不确定性重加权
完全基于原始 BCP 源码，仅在 self_train 中新增 CMC v3 不确定性分支。
...
# ================================================================
# 参数（原始 BCP 参数完全保留，仅追加 CMC v3 专用参数）
# ================================================================
parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='../data_split/ACDC')
parser.add_argument('--exp', type=str, default='BCP_CMC_v3_uncertainty')
...
parser.add_argument('--cmc_disagree_beta',   type=float, default=5.0,
                    help='JSD 衰减系数 β：β=1温和，β=5中等(推荐)，β=10激进')
...
# ================================================================
# Self-train：BCP 部分与原始完全一致，新增 CMC v3 不确定性分支
# ================================================================
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    writer = SummaryWriter(snapshot_path + '/log')
    logging.info("Start self_training (BCP + CMC v3 Uncertainty)")
    logging.info("{} iterations per epoch".format(len(trainloader)))
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # CMC v3 不确定性重加权分支（新增）
            #
            # 核心：用两互补视图的预测分歧（JSD）作为免费的不确定性估计
            #   w = teacher_conf * agree_weight
            #   agree_weight = exp(-β * JSD(p_A, p_B))
            #   JSD ∈ [0, ln2]，天然有界，比 KL 更稳定
            #
            # 梯度设计：JSD stop_gradient，仅作权重，不作优化目标
            #   若允许 JSD 梯度，模型会学到让两视图预测一致的 trivial solution
            #
            # 与 v2 的关系：v3 无新增损失项，仅改变锚点损失的像素权重
            # ==============================================================
            ...
            with torch.no_grad():
                teacher_hard_a = pre_a.argmax(dim=1).long()
            ...
            def cmc_uncertainty_loss(out_vA, out_vB, teacher_hard, conf_mask, beta):
                """
                JSD 不确定性重加权锚点损失
                w = conf_mask * exp(-β * JSD(p_A, p_B))
                loss = Σ w * [CE(pred_A, y) + CE(pred_B, y)] / Σw / 2
                JSD 梯度被截断，仅用于计算权重
                """
                with torch.no_grad():
                    p_a = F.softmax(out_vA, dim=1)          # [B,C,H,W]
                    p_b = F.softmax(out_vB, dim=1)
                    p_m = (p_a + p_b) / 2.0
                    # JSD = (KL(p_A||M) + KL(p_B||M)) / 2
                    kl_a = (p_a * (torch.log(p_a + 1e-8) - torch.log(p_m + 1e-8))).sum(dim=1)
                    kl_b = (p_b * (torch.log(p_b + 1e-8) - torch.log(p_m + 1e-8))).sum(dim=1)
                    jsd  = ((kl_a + kl_b) / 2.0).clamp(min=0.0)   # [B,H,W] ∈ [0, ln2]
                    agree_weight = torch.exp(-beta * jsd)           # [B,H,W]
                    w_combined   = conf_mask * agree_weight         # [B,H,W]
                ...
                loss = ((la + lb) * w_combined).sum() / denom / 2.0

                # 诊断统计
                jsd_mean   = jsd.mean().item()
                agree_mean = agree_weight.mean().item()
                argmax_agree = (p_a.argmax(dim=1) == p_b.argmax(dim=1)).float().mean().item()
                return loss, jsd_mean, agree_mean, argmax_agree

            loss_cmc_a, jsd_a, aw_a, aa_a = cmc_uncertainty_loss(
                out_a_viewA, out_a_viewB, teacher_hard_a, conf_mask_a, args.cmc_disagree_beta)
            loss_cmc_b, jsd_b, aw_b, aa_b = cmc_uncertainty_loss(
                out_b_viewC, out_b_viewD, teacher_hard_b, conf_mask_b, args.cmc_disagree_beta)
            ...
            jsd_mean   = (jsd_a + jsd_b) / 2
            agree_w    = (aw_a + aw_b) / 2
            argmax_ag  = (aa_a + aa_b) / 2
            ...
            # CMC v3 核心诊断指标（随训练进行 jsd↓ agree_weight↑ argmax_agree↑）
            writer.add_scalar('cmc/jsd_mean',            jsd_mean,           iter_num)
            writer.add_scalar('cmc/agree_weight_mean',   agree_w,            iter_num)
            writer.add_scalar('cmc/argmax_agree_ratio',  argmax_ag,          iter_num)
            logging.info(
                'iteration %d: loss: %f, bcp: %f, cmc: %f, '
                'jsd: %.4f, agree_w: %.3f, argmax_agree: %.3f' %
                (iter_num, loss.item(), loss_bcp.item(), loss_cmc.item(),
                 jsd_mean, agree_w, argmax_ag))
            ...
            if iter_num % 20 == 0:
                ...
                with torch.no_grad():
                    pa_v = F.softmax(out_a_viewA[:1], dim=1)
                    pb_v = F.softmax(out_a_viewB[:1], dim=1)
                    pm_v = (pa_v + pb_v) / 2.0
                    kl_a_v = (pa_v*(torch.log(pa_v+1e-8)-torch.log(pm_v+1e-8))).sum(dim=1,keepdim=True)
                    kl_b_v = (pb_v*(torch.log(pb_v+1e-8)-torch.log(pm_v+1e-8))).sum(dim=1,keepdim=True)
                    jsd_v = ((kl_a_v+kl_b_v)/2.0).clamp(min=0.0)
                    jsd_norm = jsd_v / (jsd_v.max() + 1e-8)  # 归一化到[0,1]便于可视化
                    aw_v = torch.exp(-args.cmc_disagree_beta * jsd_v.squeeze(1))
                    cw_v = (conf_mask_a[:1] * aw_v).unsqueeze(1)
                writer.add_image('cmc/JSD_Heatmap',     jsd_norm[0],        iter_num)
                writer.add_image('cmc/CombinedWeight',  cw_v[0],            iter_num)
                ...
                writer.add_image('cmc/AgreeMap', (pred_vA[0] == pred_vB[0]).float(), iter_num)
