"""
BCP + CMC-Uncertainty 半监督医学图像分割实验脚本

实验命名: BCP_CMC_uncertainty_student
指标输出: Dice, Jaccard, HD95, ASD

核心思想：互补视图不确定性重加权（CMC-Uncertainty）
=======================================================

三个 CMC 版本的演进对比：

  v1（互教版）：
    L = L_anchor + λ * L_mutual
    L_mutual：A预测监督B盲区，B预测监督A盲区
    问题：互教信号来自学生，早期质量差

  v2（融合版）：
    L = L_anchor + λ * L_fusion
    L_fusion：两视图均值预测对齐教师软标签
    问题：引入了额外损失项

  v3（本脚本，不确定性重加权版）：
    L = L_anchor_reweighted
    无新增损失项，仅改变像素权重：
      w = teacher_conf * agree_weight
      agree_weight = exp(-β * JSD(pred_A, pred_B))

    两视图分歧大（JSD 高）→ agree_weight 低 → 降低该像素监督权重
    两视图分歧小（JSD 低）→ agree_weight 高 → 正常监督

    优势：
      ① 零额外损失项，最简洁
      ② 不确定性估计来自模型自身，不依赖超参数（除 β 外）
      ③ JSD 有界 [0,ln2]，数值稳定
      ④ 与 BCP 的伪标签过滤逻辑一脉相承，易理解

关键诊断指标（TensorBoard）：
  cmc/jsd_mean           → 期望随训练下降（两视图越来越一致）
  cmc/agree_weight_mean  → 期望随训练上升（有效监督像素比例提升）
  cmc/argmax_agree_ratio → 两视图 argmax 一致比例，直觉上最易解释

参数说明（相较 v2 融合版的变化）：
  删除：--cmc_fusion_weight
  新增：--cmc_disagree_beta（JSD 衰减系数，默认 5.0）
"""
...
from utils.cmc_utils import CMCGridMaskGenerator, cmc_uncertainty_reweighted_loss
...
parser.add_argument('--exp',                 type=str,   default='BCP_CMC_uncertainty_student')
...
parser.add_argument('--cmc_loss_weight',     type=float, default=1.0,
                    help='CMC 分支总权重乘子（与 consistency_weight 相乘）')

# CMC-Uncertainty 专用参数
parser.add_argument('--cmc_disagree_beta',   type=float, default=5.0,
                    help='JSD 不确定性衰减系数 β。'
                         'β=1: 温和压制；β=5: 中等（推荐）；β=10: 激进压制')
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            if iter_num % 20 == 0:
                ...
                out_v = torch.argmax(torch.softmax(out_mixl, dim=1), dim=1, keepdim=True)
                writer.add_image('pre/Mixed_Pred',  out_v[1, ...] * 50,             iter_num)
...
# ================================================================
# Self-train 阶段：BCP + CMC-Uncertainty 联合训练
# ================================================================
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    logging.info("Loaded pre-trained from {}".format(pre_trained_model))
    ...
    writer = SummaryWriter(snapshot_path + '/log')
    logging.info("Start self_training (BCP + CMC-Uncertainty)")
    logging.info("{} iterations per epoch".format(len(trainloader)))
    logging.info(
        "CMC-Uncertainty config: patch_size={}, warmup={}, "
        "init_shared={:.2f}, cmc_loss_weight={:.2f}, beta={:.1f}".format(
            args.cmc_patch_size, args.cmc_warmup_iter, args.cmc_init_shared,
            args.cmc_loss_weight, args.cmc_disagree_beta
        )
    )
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # ============================================================
            # Part 2: CMC-Uncertainty 分支
            #
            # 流程：
            #   ① 生成互补掩码，得到两个视图
            #   ② 学生处理两视图（concat 节省 forward）
            #   ③ 计算两视图预测的 JSD → 像素级不确定性
            #   ④ w = teacher_conf * exp(-β * JSD)
            #   ⑤ 重加权锚点损失：只保留高质量、高一致性的像素的监督
            # ============================================================
            ...
            # ---- 2.7 CMC-Uncertainty 损失 ----
            # 对 uimg_a 的两个互补视图
            loss_anchor_a, stats_a = cmc_uncertainty_reweighted_loss(
                pred_a=out_a_viewA,
                pred_b=out_a_viewB,
                teacher_logit=pre_a,
            ...
            # 对 uimg_b 的两个互补视图
            loss_anchor_b, stats_b = cmc_uncertainty_reweighted_loss(
                pred_a=out_b_viewC,
                pred_b=out_b_viewD,
                teacher_logit=pre_b,
            ...
            loss_cmc = (loss_anchor_a + loss_anchor_b) / 2
            ...
            # CMC-Uncertainty 核心诊断指标
            writer.add_scalar('cmc/jsd_mean',            stats_a['jsd_mean'],            iter_num)
            writer.add_scalar('cmc/jsd_median',          stats_a['jsd_median'],          iter_num)
            writer.add_scalar('cmc/high_disagree_ratio', stats_a['high_disagree_ratio'], iter_num)
            writer.add_scalar('cmc/agree_weight_mean',   stats_a['agree_weight_mean'],   iter_num)
            writer.add_scalar('cmc/combined_weight_mean',stats_a['combined_weight_mean'],iter_num)
            writer.add_scalar('cmc/argmax_agree_ratio',  stats_a['argmax_agree_ratio'],  iter_num)
            ...
            logging.info(
                ...
                'jsd=%.4f | agree_w=%.3f | argmax_agree=%.3f | '
                'cmc_w=%.3f | shared=%.2f | conf_t=%.2f | lr=%.5f' % (
                    iter_num, loss.item(), loss_bcp.item(), loss_cmc.item(),
                    stats_a['jsd_mean'], stats_a['agree_weight_mean'],
                    stats_a['argmax_agree_ratio'],
                    cmc_weight, current_shared_ratio, current_conf_thresh, current_lr
                )
            )
            ...
            if iter_num % 20 == 0:
                ...
                # JSD 不确定性热力图（高亮分歧区域）
                with torch.no_grad():
                    pa_vis = torch.softmax(out_a_viewA[:1], dim=1)
                    pb_vis = torch.softmax(out_a_viewB[:1], dim=1)
                    pm_vis = (pa_vis + pb_vis) / 2.0
                    kl_a_vis = (pa_vis * (
                        torch.log(pa_vis + 1e-8) - torch.log(pm_vis + 1e-8)
                    )).sum(dim=1, keepdim=True)
                    kl_b_vis = (pb_vis * (
                        torch.log(pb_vis + 1e-8) - torch.log(pm_vis + 1e-8)
                    )).sum(dim=1, keepdim=True)
                    jsd_vis = ((kl_a_vis + kl_b_vis) / 2.0).clamp(min=0.0)
                    # 归一化到 [0,1] 便于可视化
                    jsd_vis_norm = jsd_vis / (jsd_vis.max() + 1e-8)
                writer.add_image('cmc/JSD_Heatmap', jsd_vis_norm[0], iter_num)

                # 组合权重图（高亮有效监督区域）
                with torch.no_grad():
                    agree_w_vis = torch.exp(
                        -args.cmc_disagree_beta * jsd_vis.squeeze(1)
                    )
                    combined_w_vis = (conf_mask_a[:1] * agree_w_vis).unsqueeze(1)
                writer.add_image('cmc/CombinedWeight', combined_w_vis[0], iter_num)

                # 两视图一致性图
                agree_vis = (pred_viewA[0] == pred_viewB[0]).float()
