"""
BCP + CMC v1：互补掩码互教一致性 — 卷积核邻居策略
基于 BCP_CMC_v1_mutual.py，新增 cmc_kernel_size 参数，
在互教损失中，将每个 exclusive patch 扩展为 kernel_size×kernel_size 的邻居区域，
使模型学习周围一圈的方格信息，类似卷积核策略。
"""
import argparse
import logging
import os
...
# ================================================================
# 参数（原始 BCP 参数完全保留，追加 CMC + conv kernel 专用参数）
# ================================================================
parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='../data_split/ACDC')
parser.add_argument('--exp', type=str, default='BCP_CMC_v1_mutual_convkernel')
...
parser.add_argument('--cmc_mutual_weight',       type=float, default=0.6)
...
# ---------- Conv Kernel 专用参数 ----------
parser.add_argument('--cmc_kernel_size',         type=int,   default=3)
...
def patients_to_slices(dataset, patiens_num):
    ...
    elif ("MM" in dataset) or ("C0" in dataset):
        ref_dict = {"1": 35, "3": 95, "5": 155, "7": 215, "10": 305}
    return ref_dict[str(patiens_num)]

# ================================================================
# CMC 掩码生成函数（同原版）
# ================================================================
def generate_cmc_masks(img, cmc_patch_size=8, shared_ratio=0.0):
...
def get_adaptive_threshold(current_iter, max_iter, init_threshold=0.90, final_threshold=0.70):
    """置信度阈值线性退火：从 init 降到 final"""
    if max_iter <= 0 or current_iter >= max_iter:
        return float(final_threshold)
    return init_threshold + (final_threshold - init_threshold) * float(current_iter) / float(max_iter)

# ================================================================
# ⭐ 卷积核 dilation 函数（新）
# ================================================================
def dilate_patch_mask(mask, patch_size, kernel_size=3):
    """
    将 exclusive mask 从单格扩展为 kernel_size×kernel_size 邻居区域。
    
    Args:
        mask: [B, H, W] 二进制掩码，1=exclusive 区域
        patch_size: CMC patch 大小
        kernel_size: 卷积核大小（如 3 = 3×3 邻居）
    
    Returns:
        [B, H, W] 扩张后的 mask
    """
    B, H, W = mask.shape
    n = H // patch_size  # grid 维度
    
    # Step 1: 下采样到 grid 空间 [B, 1, n, n]
    # 如果该格有任意 exclusive 像素，则整个格标记为 1
    mask_grid = F.avg_pool2d(mask.unsqueeze(1), 
                              kernel_size=patch_size, stride=patch_size)
    mask_grid = (mask_grid > 0.5).float()
    
    # Step 2: 用卷积核做 dilation
    # kernel: 全 1 的 kernel_size×kernel_size 核
    kernel = torch.ones(1, 1, kernel_size, kernel_size, device=mask.device)
    padding = kernel_size // 2  # 保持尺寸不变
    mask_dilated = (F.conv2d(mask_grid, kernel, padding=padding) > 0).float()
    
    # Step 3: 上采样回像素空间
    mask_dilated = F.interpolate(mask_dilated, size=(H, W), mode='nearest')
    
    return mask_dilated.squeeze(1)  # [B, H, W]
...
# ================================================================
# Self-train：BCP + CMC v1 互教 + ⭐ 卷积核邻居 dilation
# ================================================================
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    writer = SummaryWriter(snapshot_path + '/log')
    logging.info("Start self_training (BCP + CMC v1 Mutual + ConvKernel)")
    logging.info("{} iterations per epoch".format(len(trainloader)))
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            def cmc_mutual_loss_convkernel(out_viewA, out_viewB, plab_teacher,
                                           conf_mask, mask_a, mask_b):
                """
                CMC v1 互教损失 + ⭐ 卷积核 neighbor dilation。
                
                对 exclusive 掩码进行 kernel_size×kernel_size dilation，
                使每个 exclusive patch 学习其周围邻居区域的信息，
                而非仅自身一个格子。
                """
                # 锚点：两视图均对齐教师硬伪标签
                w = conf_mask
                ...
                # 互教：A 的高置信预测监督 B 的盲区（含邻居扩张），反之亦然
                with torch.no_grad():
                ...
                excl_a_raw = mask_a.squeeze(1) * (1.0 - mask_b.squeeze(1))
                excl_b_raw = mask_b.squeeze(1) * (1.0 - mask_a.squeeze(1))

                # ⭐ 关键修改：对 exclusive mask 做 conv dilation
                excl_a = dilate_patch_mask(excl_a_raw, args.cmc_patch_size,
                                           args.cmc_kernel_size)
                excl_b = dilate_patch_mask(excl_b_raw, args.cmc_patch_size,
                                           args.cmc_kernel_size)
            ...
            loss_cmc_a = cmc_mutual_loss_convkernel(out_a_viewA, out_a_viewB,
                                                     plab_teacher_a, conf_mask_a, mask_a_ab, mask_b_ab)
            loss_cmc_b = cmc_mutual_loss_convkernel(out_b_viewC, out_b_viewD,
