...
parser.add_argument('--exp', type=str, default='BCP_Multi_patch', help='experiment_name')
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                # ==========================================================
                # 修改点：将固定尺寸的盲切替换为【多尺度随机盲切 Multi-scale Patching】
                # ==========================================================
                B, C, H, W = uimg_a.shape
                num_patches = 1  # 每个图像保留的孤岛数量

                # 定义多尺度的范围，例如从 32 到 160 之间随机
                min_scale = 32
                max_scale = 160
                ...
                for i in range(B):
                    for _ in range(num_patches):
                        ...
                        y1 = max(0, center_y - current_patch_size // 2)
                        y2 = min(H, center_y + current_patch_size // 2)
                        x1 = max(0, center_x - current_patch_size // 2)
                        x2 = min(W, center_x + current_patch_size // 2)
                ...
                # === 必须同步涂黑伪标签，否则网络会在黑色背景强行预测心脏导致 Loss 崩溃 ===
                plab_a_masked = plab_a * random_patch_mask.squeeze(1).long()

                unl_label = plab_a_masked  # 保留用于后续 Tensorboard 可视化
                l_label = lab_b  # 保留用于后续 Tensorboard 可视化
                # ==========================================================
            ...
            # 修改点：改变送入网络的图像
            # ==========================================================
            net_input_unl = uimg_a * random_patch_mask  # 无标签图：只保留多尺度随机选中的 Patch，其余全黑
            ...
            # === 将同步屏蔽后的 plab_a_masked 传给 mix_loss ===
