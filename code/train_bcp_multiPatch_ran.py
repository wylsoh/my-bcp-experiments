...
parser.add_argument('--exp', type=str, default='BCP_Multiscale_Patch', help='experiment_name')
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                # ==========================================================
                # 新增模块：多尺度随机盲切 + 动态尺度置信度分配
                # ==========================================================
                B, C, H, W = uimg_a.shape
                num_patches = 1

                # 定义多尺度范围 (小尺度重细节容错，大尺度重全局约束)
                min_scale = 32
                max_scale = 160

                random_patch_mask = torch.zeros_like(uimg_a)
                batch_patch_sizes = []  # 用于记录本批次抽取的尺度集合
                ...
                # 兼容原版函数的 loss_mask 格式 (全 1)，以便无缝对接 mix_loss 计算
                loss_mask = torch.ones(img_a.shape[0], img_a.shape[2], img_a.shape[3]).cuda().long()

                # 【致命错误修复】：同步涂黑伪标签中的背景，防止计算全黑区域的错误梯度
                ...
                # 设计动态乘子：尺度最小时降权为 0.2，最大时升权为 1.5
                dynamic_conf_multiplier = 0.2 + 1.3 * scale_ratio

                # 结合原始设置的基础 u_weight，得出当前步真实的无标签约束权重
                dynamic_u_weight = args.u_weight * dynamic_conf_multiplier
                # ==========================================================

            consistency_weight = get_current_consistency_weight(iter_num // 150)

            # 网络输入更替：无标签图使用随机盲切，有标签图使用干净全图
            ...
            # 使用经过遮罩处理的伪标签 plab_a_masked 和 动态生成的 dynamic_u_weight
