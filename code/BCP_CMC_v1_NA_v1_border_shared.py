"""
BCP + CMC v1 - NA-CMC V1：邻域感知互补掩码（边界共享带）
=============================================================
改动说明（相对于原始 BCP_CMC_v1_mutual.py）：
  ① generate_cmc_masks → generate_cmc_masks_v1_border
     - 在 patch 空间做 max_pool2d（形态学膨胀），使边界 patch 在两个视图均可见
     - 新增 border_width 参数（默认 1），代表向外扩张几圈 patch
  ② cmc_mutual_loss 内部不变，border_mask 仅用于日志监控
  ③ 新增 argparse 参数：--cmc_border_width（默认 1）

其余所有代码与原始脚本完全一致，未做任何其他修改。

快速验证方法：
  python BCP_CMC_v1_NA_v1_border_shared.py --labelnum 7 --cmc_border_width 1 --exp BCP_CMC_NA_v1_bw1
  python BCP_CMC_v1_NA_v1_border_shared.py --labelnum 7 --cmc_border_width 0 --exp BCP_CMC_baseline
  对比两组实验的 val_mean_dice 和 val_*_hd95 曲线
"""
import argparse
import logging
import os
...
# ================================================================
# 参数（原始 BCP 参数完全保留，新增 cmc_border_width）
# ================================================================
parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='../data_split/ACDC')
parser.add_argument('--exp', type=str, default='BCP_CMC_NA_v1')
...
# ---------- NA-CMC V1 新增参数 ----------
parser.add_argument('--cmc_border_width', type=int, default=1,
    help='边界膨胀宽度（patch 数量），0 = 关闭，等价于原始 CMC；1 = 向外扩 1 圈 patch')
...
# ================================================================
# NA-CMC V1 核心改动：带边界膨胀的掩码生成
# ================================================================
def generate_cmc_masks_v1_border(img, cmc_patch_size=16, shared_ratio=0.0, border_width=1):
    """
    NA-CMC V1：邻域感知互补掩码（边界共享带方案）

    在 patch 空间对每个掩码做 max_pool2d（形态学膨胀），使得：
    - 每个可见 patch 向外扩张 border_width 圈，覆盖相邻 patch 的边界区域
    - 两视图在 patch 边界处产生重叠带（border zone），强迫模型学习跨边界上下文
    - border_width=0 时退化为原始 CMC，与原版完全等价

    原理类比：等价于在 patch 空间施加一个 (2*border_width+1)^2 的卷积核，
    使每个遮挡区域边界处的像素也能"看到"邻居 patch 的信息。

    Args:
        img          : [B, C, H, W]
        cmc_patch_size: patch 大小（像素）
        shared_ratio : 热身阶段共享比例
        border_width : 边界膨胀宽度（patch 数），0=原始 CMC

    Returns:
        mask_a : [B, 1, H, W] float {0,1}，视图A可见区域（含边界带）
        mask_b : [B, 1, H, W] float {0,1}，视图B可见区域（含边界带）
    """
    B, C, H, W = img.shape
    n = H // cmc_patch_size  # 每行/列 patch 数，如 256/16=16
    masks_a, masks_b = [], []

    # max_pool 核参数（在 patch 空间操作）
    k = border_width * 2 + 1  # 核大小：border_width=1 → k=3（3x3 膨胀）
    pad = border_width

    for _ in range(B):
        base = (torch.rand(n, n) > 0.5).float()  # patch 级随机二值分配
        ...
        if border_width > 0:
            # --- 核心：在 patch 空间做形态学膨胀 ---
            # max_pool2d 对二值掩码 = 膨胀操作
            # pa_exp: [1,1,n,n] → max_pool → [1,1,n,n]（每个1值向外扩 border_width 圈）
            pa_exp = F.max_pool2d(pa.view(1, 1, n, n), kernel_size=k,
                                  stride=1, padding=pad)
            pb_exp = F.max_pool2d(pb.view(1, 1, n, n), kernel_size=k,
                                  stride=1, padding=pad)
        else:
            # border_width=0：退化为原始 CMC
            pa_exp = pa.view(1, 1, n, n)
            pb_exp = pb.view(1, 1, n, n)

        # 上采样到原始像素分辨率（nearest 保持边界清晰）
        pa_full = F.interpolate(pa_exp, size=(H, W), mode='nearest').squeeze(0)  # [1,H,W]
        pb_full = F.interpolate(pb_exp, size=(H, W), mode='nearest').squeeze(0)  # [1,H,W]

        masks_a.append(pa_full)
        masks_b.append(pb_full)
...
# ================================================================
# Self-train：BCP 部分不变，CMC 分支使用新掩码生成函数
# ================================================================
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    writer = SummaryWriter(snapshot_path + '/log')
    logging.info("Start self_training (BCP + NA-CMC V1 Border Shared)")
    logging.info("border_width={}  →  每个 patch 向外扩张 {} 圈，边界带宽 {} px".format(
        args.cmc_border_width, args.cmc_border_width, args.cmc_border_width * args.cmc_patch_size))
    logging.info("{} iterations per epoch".format(len(trainloader)))
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # ① 使用带边界膨胀的掩码生成（唯一改动）
            mask_a_ab, mask_b_ab = generate_cmc_masks_v1_border(
                uimg_a, args.cmc_patch_size, shared_ratio, args.cmc_border_width)
            mask_a_cd, mask_b_cd = generate_cmc_masks_v1_border(
                uimg_b, args.cmc_patch_size, shared_ratio, args.cmc_border_width)
            ...
            def cmc_mutual_loss(out_viewA, out_viewB, plab_teacher,
                ...
                # 注意：由于 mask_a/b 已包含膨胀边界带，
                # excl_a/b 的范围自动缩小（边界像素两视图均可见，不再是独占区域）
            ...
            # 监控：计算当前边界带覆盖率（用于 tensorboard 观察）
            with torch.no_grad():
                border_zone = (mask_a_ab.squeeze(1) * mask_b_ab.squeeze(1))
                border_ratio = border_zone.float().mean().item()
            ...
            # NA-CMC V1 新增监控：边界带面积占比
            writer.add_scalar('na_cmc/border_ratio',    border_ratio,       iter_num)
            logging.info(
                'iter %d | loss=%.4f bcp=%.4f cmc=%.4f | border_ratio=%.3f shared=%.2f conf_t=%.2f' %
                (iter_num, loss.item(), loss_bcp.item(), loss_cmc.item(),
                 border_ratio, shared_ratio, current_conf_thresh))

            if iter_num % 20 == 0:
                writer.add_image('train/Un_Image',
                                 net_input_unl[1, 0:1, :, :], iter_num)
                ...
                # 可视化边界带（两视图重叠区域）
                border_vis = border_zone[0:1].float()
                writer.add_image('cmc/BorderZone', border_vis, iter_num)
