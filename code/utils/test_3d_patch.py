...
def var_all_case_LA(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
    # 本文件在 utils/ 下，LA 数据在 ../data_split/LA/
    base_dir = os.path.join(os.path.dirname(__file__), '..', '..', 'data_split', 'LA')
...
def var_all_case_LA_plus(model_l, model_r, num_classes,
    ...
    for image_path in loader:
        ...
        prediction, score_map = test_single_case_plus(
            model_l, model_r, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
        if np.sum(prediction) == 0:
            dice = 0
        else:
            dice = metric.binary.dc(prediction, label)
...
def test_all_case(model, image_list, num_classes, patch_size=(112, 112, 80),
