...
parser.add_argument('--exp', type=str, default='BCP_random_patch', help='experiment_name')
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                # 在内部局部定义，防止覆盖 DataLoader 的输入分辨率
                mask_patch_size = 32  # 孤岛的大小
                num_patches = 1  # 随机保留的孤岛数量
                ...
                # 伪造原版函数的 mask 格式 (全1)，无缝兼容后面坚决不改的 mix_loss
                img_mask = torch.ones(img_a.shape[2], img_a.shape[3]).cuda().long()
                loss_mask = torch.ones(img_a.shape[0], img_a.shape[2], img_a.shape[3]).cuda().long()

                # === 致命 BUG 修复处：伪标签掩码同步 ===
                # 图像变黑的地方，伪标签也必须被涂黑归零，否则强行计算 Loss 会导致崩溃
                plab_a_masked = plab_a * random_patch_mask.squeeze(1).long()

                # 保留用于后续 Tensorboard 可视化 (看这里，显示的是切分后的干净标签块)
            ...
            # 修改点：改变送入网络的图像
            # ==========================================================
            net_input_unl = uimg_a * random_patch_mask  # 无标签图：只保留随机选中的 Patch，其余全黑
            net_input_l = img_b  # 有标签图：保持原样提供干净监督
            # ==========================================================
            out_unl = model(net_input_unl)
            out_l = model(net_input_l)

            # === 这里也必须传入修复后的 plab_a_masked ===
            unl_dice, unl_ce = mix_loss(out_unl, plab_a_masked, lab_a, loss_mask, u_weight=args.u_weight, unlab=True)
...
if __name__ == "__main__":
    ...
    shutil.copy(__file__, self_snapshot_path)  # 将 '.' 改为了 __file__ 以防止目录错误
    ...
    if not os.path.exists(os.path.join(pre_snapshot_path, '{}_best_model.pth'.format(args.model))):
