"""
消融实验: 移除 BCP 双向 Copy-Paste, 仅保留 MAE 分支

对比目的:
  原方法: BCP_MAE (BCP + MAE 联合)
  消融版: MAE_only (仅 MAE 分支 + 有标签标准监督)

实验命名: MAE_only_ablation
...
parser.add_argument('--exp', type=str, default='MAE_only_ablation', help='实验名称(消融版)')
...
parser.add_argument('--mae_warmup_iter', type=int, default=3000, help='MAE遮挡渐进热身步数')
...
# ================================================================
# 标准监督损失 (替代消融掉的 BCP mix_loss)
# ================================================================
def supervised_loss(output, label):
    """
    标准有监督分割损失 (Dice + CE)
    用于消融版本中替代 BCP 的 mix_loss

    Args:
        output: [B, C, H, W] 网络logits
        label:  [B, H, W]    真实标签
    Returns:
        loss_dice, loss_ce
    """
...
# ================================================================
# Pre-train 阶段（保持不变, 仍用 BCP copy-paste）
# 说明: 仅消融 self-train 阶段的 BCP, pre-train 不动
# ================================================================
def pre_train(args, snapshot_path):
    ...
    logging.info("Start pre_training (standard BCP, unchanged)")
...
# ================================================================
# Self-train 阶段: 消融 BCP, 仅保留 MAE
#
# 原版流程:
#   有标签 + 无标签 → BCP双向copy-paste混合 → mix_loss
#                  → MAE遮挡(学生) + 完整(教师) → mae_consistency_loss
#
# 消融版流程:
#   有标签 → 直接监督损失 (Dice + CE)
#   无标签 → MAE遮挡(学生) + 完整(教师) → mae_consistency_loss
# ================================================================
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    writer = SummaryWriter(snapshot_path + '/log')
    logging.info("=" * 60)
    logging.info("ABLATION: BCP copy-paste removed, MAE-only branch")
    logging.info("=" * 60)
    logging.info("{} iterations per epoch".format(len(trainloader)))
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # ============================================================
            # 【消融】Part 1: 有标签数据 → 标准监督损失 (替代 BCP mix_loss)
            # ============================================================
            # 拼接所有有标签数据
            labeled_img = torch.cat([img_a, img_b], dim=0)
            labeled_gt = torch.cat([lab_a, lab_b], dim=0)

            out_labeled = model(labeled_img)
            sup_dice, sup_ce = supervised_loss(out_labeled, labeled_gt)
            loss_sup = (sup_dice + sup_ce) / 2
            ...
            # Part 3: 总损失 (消融版)
            #
            # 原版: loss = loss_bcp + consistency_weight * mae_weight * loss_mae
            # 消融: loss = loss_sup + consistency_weight * mae_weight * loss_mae
            # ============================================================
            mae_rampup = min(1.0, iter_num / max(args.mae_warmup_iter, 1))
            mae_weight = args.mae_loss_weight * mae_rampup

            loss = loss_sup + consistency_weight * mae_weight * loss_mae
            ...
            writer.add_scalar('info/loss_sup_dice', sup_dice, iter_num)
            writer.add_scalar('info/loss_sup_ce', sup_ce, iter_num)
            ...
            logging.info(
                'iteration %d: loss: %f, sup: %f, mae: %f, '
                'mae_w: %f, mask_r: %.2f, conf_t: %.2f, '
                'conf_a: %.3f, conf_b: %.3f, lr: %f' %
                (iter_num, loss.item(), loss_sup.item(), loss_mae.item(),
            ...
            if iter_num % 20 == 0:
                # 有标签监督分支
                image_l = labeled_img[0, 0:1, :, :]
                writer.add_image('train/Sup_Image', image_l, iter_num)
                outputs_l = torch.argmax(torch.softmax(out_labeled, dim=1), dim=1, keepdim=True)
                writer.add_image('train/Sup_Prediction', outputs_l[0, ...] * 50, iter_num)
                labs_l = labeled_gt[0, ...].unsqueeze(0) * 50
                writer.add_image('train/Sup_GroundTruth', labs_l, iter_num)
        ...
        writer.close()
        logging.info("=" * 60)
        logging.info("Ablation Self-training (MAE-only) finished.")
