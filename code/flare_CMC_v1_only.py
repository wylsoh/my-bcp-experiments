"""
Flare CMC v1 only：去掉 BCP mix 分支，只保留 CMC 互教一致性（3D，两阶段）
...
def cmc_mutual_loss_3d(out_A, out_B, teacher_hard, conf_mask, mask_a, mask_b):
    ...
    excl_a = mask_a.squeeze(1) * (1.0 - mask_b.squeeze(1))  # 仅A可见
...
def build_loader():
    ...
    db_train = BraTS2019(
        base_dir=train_data_path, split='train', num=None,
        transform=transforms.Compose([
            RandomRotFlip(), RandomCrop(patch_size), RandomNoise(), ToTensor()
        ])
    )
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            if iter_num % 200 == 0:
                ...
                dice_val = float(avg_metric[:, 0].mean())
...
# python flare_CMC_v1_only.py --labeled_num 21 --gpu 0